...
def main():
    ...
    if previous.exists():
        ...
        unchanged = old['source_sha256'] == current['source_sha256'] and old['issues'] == current['issues'] and old.get('native') == current.get('native') and old.get('comparison') == current.get('comparison') and old.get('performance_plan') == current.get('performance_plan') and old.get('pilot') == current.get('pilot')
